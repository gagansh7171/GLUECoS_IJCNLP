...
if [ $TASK == 'LID_EN_ES' ]; then 
  ...
  bash $CODE_DIR/train_sentence2.sh "Sentiment_EN_ES" $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
  bash $CODE_DIR/train_sentence2.sh "Sentiment_EN_HI/Devanagari" $MODEL $MODEL_TYPE $DATA_DIR $OUTPUT_DIR
